...
import { postJobApi } from '../Redux/JobPostSlice';
...
const PostJob = () => 
{
    ...
    const validation = () => {
        ...
        if(user.logo==='')
        {
            error.logo='Company Logo is required';
        }
        if(user.document==='')
        {
    ...
    const postUserData = e =>
    {
        ...
        if (name === 'jobTitle') {
            ...
            if (name === 'logo') {
                if (value.length === 0) {
                    setError({ ...error, logo: '@Company Logo is Required' });
                    setUser({ ...user, logo: '' });
                }
                else {
                    setError({ ...error, logo: '' });
                    setUser({ ...user, logo: e.target.files[0]?.name });
                }
            }
            if (name === 'document') {
                if (value.length === 0) {
                    setError({ ...error, document: '@Company document is Required' });
                    setUser({ ...user, document: '' });
                }
                else {
                    setError({ ...error, document: '' });
                    setUser({ ...user, document: e.target.files[0]?.name });
                }
            }
        ...
        const handleSubmit = async e => {
            ...
            if (Object.keys(errorList).length < 1) {
                //Api call start... 
                const res = await postJobApi(user);
                ...
                <section id="post_job">
                    ...
                    <div className="container" style={{ boxShadow: '30px 30px 50px', padding: '30px', marginBottom: '80px', marginLeft: '80px', borderRadius: '20px' }}>
                        <div className="list-box text-center" style={{ width: '600px' }}>
                            <h3 className="font-color-black margin-right" style={{ color: 'orange' }}>Job Post Form</h3>
                        </div>
                        ...
                        <div className="job-post-box">
                            <form>
                                ...
                                <div className="row">
                                    <div className="col-lg-6 col-md-6">
                                        <div className="form-group ">
                                            ...
                                            <div className="box text-center">
                                                <input type="file" name="logo" id="file-5"   autoComplete='on' required onChange={postUserData} />
                                    ...
                                    <div className="col-lg-6 col-md-6">
                                        <div className="form-group">
                                            ...
                                            <div className="box text-center">
                                                <input type="file" name="document" id="file-7"  autoComplete='on' required onChange={postUserData} />
                                                <span className='text-danger'>{error?.document}</span>
                                ...
                                <div className="row">
                                            ...
                                            <div className="row">
                                                <div className="col-lg-4 col-md-4">
                                                    <div className="form-group">
                                                        ...
                                                        <select className="form-control" id="sel1" name="jobNature" onChange={postUserData}>
                                    ...
                                    <button type="submit" className="btn Post-Job-Offer" onClick={handleSubmit}>Post Job Offer</button>
                                </div>
                            </form>
                        </div>
                    </div>
